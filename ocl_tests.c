...
cl_int test_layer(struct GeneratorContext* context, LayerStack* stack, struct Error* errors, int* error_count, int* bufferB, int layer) {
    ...
    err = generate_layer(context, layer, dims, bufferB, NULL, &event);
    if (err < 0) {
...
cl_int run_tests(struct Error* errors, int* error_count) {
    ...
    cl_int err = init_generator_context(&context, MC_1_16, W, H);
    if (err < 0) return err;
    cl_event event0;
    set_world_seed(&context, seed, &event0);
    clWaitForEvents(1, &event0);

    cl_int* bufferB = (cl_int*) malloc(W * H * sizeof(cl_int));
...
int main() {
    ...
    if (error_count > 0) {
        ...
        for (int i = 0; i < error_count; ++i) {
            printf("x=%d z=%d \t| expected=%d \treal=%d\n", errors[i].x, errors[i].z, errors[i].expected, errors[i].ocl);
        }
    } else if (err == 0) {
        printf("No errors.\n :)\n");
    }
